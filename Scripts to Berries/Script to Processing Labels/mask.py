...
def Ellipse2Mask(ellipse, n):
    ...
    i = 0
    while dt*i < 2*np.pi:
        x = round(ellipse["rx"]*np.cos(i*dt)*np.cos(ellipse["theta"]) -
                  ellipse["ry"]*np.sin(i*dt)*np.sin(ellipse["theta"]) + ellipse["cx"])
        y = round(ellipse["rx"]*np.cos(i*dt)*np.sin(ellipse["theta"]) +
                  ellipse["ry"]*np.sin(i*dt)*np.cos(ellipse["theta"]) + ellipse["cy"])
...
for k, v in data.items():
    ...
    img_data = {'file_name': img_path, 'image_id': int(k.split(
        ".")[0]), 'height': img.shape[0], 'width': img.shape[1], 'annotations': []}
    ...
    for i in v["regions"]:
        ...
        img = cv2.rectangle(img, (bbox["x"], bbox["y"]), (
            bbox["x"]+bbox["width"], bbox["y"]+bbox["height"]), (255, 0, 0), thickness=3)
        mask = Ellipse2Mask(el, 30)

        annotation = {'iscrowd': 0, 'segmentation': [flatten(mask)], 'bbox': [
            bbox["x"], bbox["y"], bbox["width"], bbox["height"]], 'bbox_mode': detectron2.structures.boxes.BoxMode.XYWH_ABS, 'category_id': 0}
        img_data["annotations"].append(annotation)

        img = cv2.polylines(img, np.int32(
            [np.array(mask)]), True, (0, 255, 0), thickness=3, lineType=cv2.LINE_8)
